...
예시:  
```js
R.id.username = $("legend", {text: "hynrusang"});
const firstFragment("fragmentView", $("section").add(
    R.id.username,
    $("input", {type: "text"}),
    $("input", {type: "button", value: "user name edit.", onclick: () => {
        R.id.username.set({text: scan("input").value});
    }}),
    $("input", {type: "button", value: "go to second Fragment", onclick: () => {
        secondFragment.launch()
    }})
)).registAnimation(FragAnimation.card, 0.8);
const secondFragment("fragmentView", $("section").add(
    R.id.username,
    $("input", {type: "button", value: "go to first Fragment", onclick: () => {
        firstFragment.launch();
    }})
)).registAnimation(FragAnimation.fade, 0.8);
```
> 이렇게 하면 **R.id.username** 객체가 **firstFragment**와 **secondFragment** 객체에서 **재사용됩니다.**  
---
## 업데이트 내역
> 1.0.0  
